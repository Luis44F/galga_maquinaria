...
<head>
    ...
    <style>
        * {
        ...
        /* Texto más claro para categorías y detalles */
        td small.text-muted,
        td .text-muted {
        ...
        /* Mejor contraste para el texto principal de la tabla */
        .table td {
            color: #f1f5f9;
        }

        /* Texto de categoría (TEJEDORAS, BORDADORAS, etc.) */
        td small {
            color: #cbd5e1 !important;
            font-weight: 500;
        }
        ...
        /* Precio de compra */
        td .text-muted strong {
            color: #94a3b8;
            font-weight: 600;
        }

        /* Hover en filas */
        tr:hover td small.text-muted,
        tr:hover td .text-muted {
...
<body>
    ...
    <script>
        ...
        function cargarMaquinaria() {
            ...
            let url = '/maquinaria-disponible-test?';
            const params = [];
        ...
        function cargarEstadisticasMaquinaria() {
            fetch('/maquinaria-disponible-test/estadisticas/resumen', {
                headers: { 'X-Requested-With': 'XMLHttpRequest' }
            })
            .then(response => response.json())
            .then(data => {
        ...
        function cambiarEstado() {
            ...
            fetch(`/maquinaria-disponible-test/${id}/cambiar-estado`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': csrfToken
        ...
        function reservarMaquina(id) {
            if (confirm('¿Reservar esta máquina para un cliente?')) {
                fetch(`/maquinaria-disponible-test/${id}/reservar`, {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': csrfToken
                    }
        ...
        function venderMaquina(id) {
            if (confirm('¿Confirmar venta de esta máquina?')) {
                fetch(`/maquinaria-disponible-test/${id}/vender`, {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': csrfToken
                    }
